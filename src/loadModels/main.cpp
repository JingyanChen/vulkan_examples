...
/*
 * GLM is designed for OGL which depth range[-1,1]
 * we shoule force it into [0,1]
 */
 ...
 * 
 * now in this case , we will use 3d pos data 
 * 
 * 1 change vertexInputInfo(format foram R32G32 to RGB32) in pipeline object slots and of course extent 2D pos data into 3D 
 * 2 modify vertex shader to accept 3d pos 
 * 3 now we create two squre by add vetex data with different z 
 * 4 create a vkImage with memory for depth usage(VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT and corresponding depth format) 
 *   and transfer vkImage layout into VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
 *   of course we should warp vkImage into vkImage view see function createDepthResources
 * 5 add a new attachment point called detph attachement , when we create render pass and subpass 
 *   we should give in a depth attachement index see VkAttachmentReference depthAttachmentRef{}
 * 6 when we tell subpass a new depth attachement point (1 in this case) , we should link depth vkImage as a
 *   source in framebuffer->pAttachments , take a vkImageView as a depth attachement into framebuffer corresponding position
 *   see createFramebuffer diff
 * 7 we should point depth test configure when we create a pipeline object and set up a VkPipelineDepthStencilStateCreateInfo
 *   configure and setup into pipeline object -> pDepthStencilState slots
 *  8 now depth test is completely enable
 * 
 */

static vkGraphicsDevice * vulkanDevice = new vkGraphicsDevice(800, 600);
vulkanShader* vs = new vulkanShader("../src/loadModels/meta/vs.spv", VK_SHADER_STAGE_VERTEX_BIT);
vulkanShader* ps = new vulkanShader("../src/loadModels/meta/ps.spv", VK_SHADER_STAGE_FRAGMENT_BIT);

const std::string MODEL_PATH = "../obj/viking_room.obj";
const std::string TEXTURE_PATH = "../obj/viking_room.png";
...
/*
 * in this case we will get vertices and indices from 
 * model file
 */
...
#define CREATE_DEPTH_BUFFER_RELATED_CODE
...
void createTextureImage() {
    ...
    /*loading a texture from model file*/
...
void createRenderPass() {
    ...
    /*unlike color attachment, a subpass can only use a single depth stencil attachement*/
    subpass.pDepthStencilAttachment = &depthAttachmentRef;

    /*
     * point out dependency with different subpass
     * in this case , we only use single subpass , so this option is ignore.
     * select dependency for depth attachent in this case
     */
     ...
     * 1. attachement format stored in colorAttachment and depth attachment
     ...
     */
    //now we have two attchament one color and one depth 
...
#ifdef CREATE_DEPTH_BUFFER_RELATED_CODE
/*
...
void createFramebuffer() {
    ...
    /*
     * bind depth vkimageView into framebuffer
     */
...
void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    /*
     * before we start renderpass , we can select it's attachement clear color
     * in this case , we can clear depth attachement as 1.0
     * 
     * note that the order of clearValues should be identiacal to the order of your attachment
     * now attchment order is color , depthStencil
     */
...
/*
 * load vertex and indices data from model file
 * OBJ file consists of positions , normals , texture coordinates and faces.
 * faces consist of an arbitrary amount of vertices, where each vertex refers to a position ,
 * normal and/or texture corrdinate by index
 * 
 * actually data is attrib.vertices,attrib.normals,attrib.textcoords
 */
void loadModel() {
